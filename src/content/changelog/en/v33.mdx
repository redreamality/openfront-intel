---
title: 'v33.9: Desktop & Server Reliability, plus v33 Rules'
description: 'Player-focused OpenFront v33.9 notes: understand the desktop and server reliability patches, plus the v33.7 gameplay rules still in force.'
pubDate: 2026-07-31
...
## Direct answer: what changed in v33.9 and v33.8?

The official `v0.33.9` Release adds the release descriptor used by the desktop client. The preceding `v0.33.8` patch routes the game server's HTTP API through its configured host. Both are delivery and reliability changes; neither Release declares a new control, balance value, map rule, or match decision.

For your next game, the latest player-facing changes remain the spectator and Team Doomsday rules released in v0.33.7. You do not need to change a build order or combat timing just because the displayed patch moved from v33.7 to v33.9.
...
GitHub labels the official `v0.33.9` Release `draft=false` and `prerelease=false`; its body contains real v33.9 through v33.0 entries rather than TEST placeholder text. The v33.9 and v33.8 entries are reliability patches, while the v0.33.7 tag remains the source for the latest gameplay changes. This remains the single v33 series overview.

Sources: [official v0.33.9 Release](https://github.com/openfrontio/OpenFrontIO/releases/tag/v0.33.9), [official v0.33.7 gameplay Release](https://github.com/openfrontio/OpenFrontIO/releases/tag/v0.33.7), [spectator lobby flow](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.7/src/client/JoinLobbyModal.ts), [Doomsday ladders](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.7/src/core/game/DoomsdayClock.ts), [wasteland rot](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.7/src/core/execution/DoomsdayClockExecution.ts), [versioned replay URL helper](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.6/src/client/VersionedReplay.ts), [archived replay redirect](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.6/src/client/JoinLobbyModal.ts), [Luna rotation frequency](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.6/src/core/game/Maps.gen.ts), [adminbot game creation](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.5/src/server/AdminBotRoutes.ts), [bulk limits and costs](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.4/src/core/game/Game.ts), and [same-Silo launch scheduling](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.4/src/core/execution/NukeExecution.ts).
