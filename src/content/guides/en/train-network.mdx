---
title: "OpenFront Train Network: Rail Range, Stops, and Payback"
description: "Build a connected OpenFront rail network with the 110-tile station search, 155.6-tile segment limit, stop payouts, and clear exit signals."
pubDate: 2026-08-28
...
difficulty: Advanced
version: v34
freshnessSummary: "Rechecked on 2026-09-17 against the formal v0.34.7 tag: Factory-count spawn rate is adjusted by global Train saturation; v0.34.5 moved its damping midpoint from 300 to 500 units. Station search remains 110 tiles and a rail segment stays below about 155.6 tiles."
---

## Direct answer: build a connected rail loop, not a pile of Factories

This page owns the **railway network**: station range, segment geometry, hubs, branches, stop payouts, and the point where another Factory stops being justified. It is not a generic Factory build order and it does not replace the Port-versus-Factory payback comparison.

The result is simple: build only when a connected graph can produce repeated paying stops, then scale the measured bottleneck. A moving Train is not proof of income; a completed settlement at a valid City or Port is.
...
The current Factory generation rule starts with `(factoryCount + 10) * 15`, then divides that rate parameter by `trainSaturation(globalTrainUnits)` and floors it, with a minimum of one. The tagged v0.34.5 patch moved the damping midpoint from 300 to 500 global Train units, so mid-to-late-game spawning is less suppressed than before; the curve still has a plateau and hard tail cap. A smaller returned rate makes a random generation attempt more likely, not a guaranteed timetable. Each Factory level contributes another attempt. A level-3 Factory in a network with four Factories has more local attempts than a level-1 Factory, but all of them still need an eligible station and a path that can complete.
...
For the team-space decision before a Factory purchase, continue with the [Team economy space guide](/guides/team-economy-space/) to reserve the corridor, assign the payer, and verify who protects each station.

## Train network FAQ

- **What does this page answer?** It answers how a railway network connects Factory sources to paying City and Port stations, how to measure stops, and when to stop investing. It does not promise a universal Factory build order.
- **What distances must I check?** Stations search within 110 tiles, gaps below 15 tiles are skipped, and each usable segment must stay below about 155.6 tiles. Measure the actual path, not only a straight line.
- **Why can Trains move without earning Gold?** A Train can spawn while ownership, diplomacy, route geometry, or station validity prevents a paying stop. Count completed settlements, not moving icons.
- **When should I stop upgrading?** Stop after the test window if three complete arrivals do not materialize, if the next stop is diluted after ten, or if one capture or embargo can remove the only paying station. Repair the graph or leave with reserve Gold.

## Official sources and the v0.34.7 boundary

The current spawn amendment is documented in the [formal v0.34.5 Release](https://github.com/openfrontio/OpenFrontIO/releases/tag/v0.34.5) and verified on 2026-09-17 against tagged [v0.34.7 Config.ts](https://github.com/openfrontio/OpenFrontIO/blob/v0.34.7/src/core/configuration/Config.ts) and [TrainStationExecution.ts](https://github.com/openfrontio/OpenFrontIO/blob/v0.34.7/src/core/execution/TrainStationExecution.ts). The base `(factoryCount + 10) * 15` is not the complete returned rate: global Train-unit saturation changes it, and a Factory's level controls repeated attempts. The v0.34.5 midpoint change is a population-dependent spawn adjustment, not a guaranteed number of trains or Gold per minute. Keep route and settlement observations separate from the code parameter.
...
The original numeric route facts come from the tagged [v0.33.12 Config.ts](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.12/src/core/configuration/Config.ts), [FactoryExecution.ts](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.12/src/core/execution/FactoryExecution.ts), [TrainStationExecution.ts](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.12/src/core/execution/TrainStationExecution.ts), [TrainExecution.ts](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.12/src/core/execution/TrainExecution.ts), [TrainStation.ts](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.12/src/core/game/TrainStation.ts), [RailNetworkImpl.ts](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.12/src/core/game/RailNetworkImpl.ts), and [CityExecution.ts](https://github.com/openfrontio/OpenFrontIO/blob/v0.33.12/src/core/execution/CityExecution.ts). The current [v0.34.7 Config.ts](https://github.com/openfrontio/OpenFrontIO/blob/v0.34.7/src/core/configuration/Config.ts) and [TrainStationExecution.ts](https://github.com/openfrontio/OpenFrontIO/blob/v0.34.7/src/core/execution/TrainStationExecution.ts) confirm the shared Factory count base, global Train-unit saturation, one attempt per Factory level, 110-tile search, approximately 155.6-tile segment cap, and station-stop payout schedule. The formal [v0.34.5 Release](https://github.com/openfrontio/OpenFrontIO/releases/tag/v0.34.5) names the mid-to-late-game spawn increase. A route's actual income remains an observation, not an output of the spawn formula alone.
...
For version-specific timing after v0.34.12's spawn-curve change, read the [Port and Factory timing guide](/guides/frontload-port-factory-investment/).
