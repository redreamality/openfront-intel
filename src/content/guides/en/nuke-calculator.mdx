---
title: "OpenFront Nuke Calculator: MIRV Cost, Silo Slots & SAM Range"
description: "Use the OpenFront nuke calculator for exact Atom, Hydrogen, and MIRV costs, ready Silo and SAM slots, SAM range, and a bounded pressure estimate."
pubDate: 2026-08-22
...
difficulty: Advanced
version: v34
freshnessSummary: "Verified against v0.34.11: MIRVs start at 25M, rise by 15M per game-wide launch, and have no global launch cooldown."
---
...
**Result first:** enter your Gold, planned weapons, ready Silo tubes, and ready SAM shots. The tool returns exact costs, slot capacity, SAM range, and a labeled pressure gap.

That result answers budget and capacity. It does not simulate a live map, flight paths, interception order, casualties, or a guaranteed breakthrough.

This calculator gives exact arithmetic for ordinary Gold costs, MIRV price-ladder plans, ready Missile Silo slots, ready SAM shots, and SAM range by level.
...
The formal player-facing boundary is [v0.34.11](https://github.com/openfrontio/OpenFrontIO/releases/tag/v0.34.11). The fixed-price and global cooldown experiment in beta1 was reverted before the stable v0.34.0 release.
...
Enter how many Atom Bombs, Hydrogen Bombs, and MIRVs you want to compare. Also enter the number of MIRVs all players have already launched this game; each planned launch advances the price by 15M.
...
Inputs are planning values, not a connection to a live match. Recheck ready slots and the global launch count whenever any player fires a MIRV.
...
The next MIRV costs 25,000,000 plus 15,000,000 for every MIRV already launched in this game. There is no global launch lockout in v0.34.11.
...
## MIRV sequence cost and the global counter

Let `n` be the game-wide number of prior launches and `k` the number of planned MIRVs. Assuming nobody else fires between yours:

```text
MIRV total = k × (25,000,000 + 15,000,000 × n)
           + 15,000,000 × k × (k - 1) / 2
```

From zero prior launches, one costs 25M; two cost 65M total; three cost 120M; four cost 190M; and five cost 275M.

The calculator assumes consecutive launches. If another player fires between yours, update `n`: each intervening launch adds 15M to every remaining MIRV.

All players share this price counter, but there is no global launch cooldown. Each player still needs Gold and a ready Silo tube.

The normal 90-tick Silo-tube reload remains separate. The price calculation does not make a cooling tube ready or turn a planned sequence into one atomic action.
...
The Atoms cost 1.5M. The Hydrogen costs 5M. With zero prior MIRV launches, three MIRVs cost 120M. Total weapon cost is 126.5M, leaving 73.5M.
...
If another player launches first, the three planned MIRVs now cost 40M + 55M + 70M = 165M. Update the counter before committing.

Three separate ready tubes can support the sequence; each launch advances the price counter for everyone.
...
- Enter the game-wide MIRV launch count and budget each planned MIRV at its own rung.
- Recheck the price after any player's launch.
...
- Recalculate when another player launches or a Silo tube reloads.
...
### Treating the first price as the whole sequence

The 25M opening price applies only when the global counter is zero. Two consecutive MIRVs cost 65M, not 50M; there is no global 60-second wait in the current release.
...
The calculator handles Gold and ready local tubes, but the live game may have advanced its shared MIRV counter after you entered it. Check the current price and whether your Silo tube has reloaded.
...
The current formal release boundary is [v0.34.11](https://github.com/openfrontio/OpenFrontIO/releases/tag/v0.34.11).

The formulas are verified against the exact v0.34.11 sources and formal tests:

- [Config.ts: weapon costs, reloads, speed, radius, and SAM range](https://github.com/openfrontio/OpenFrontIO/blob/v0.34.11/src/core/configuration/Config.ts)
- [MIRVExecution.ts: launch recording and target generation](https://github.com/openfrontio/OpenFrontIO/blob/v0.34.11/src/core/execution/MIRVExecution.ts)
- [NukeExecution.ts: curved flight, targetable segments, and impact](https://github.com/openfrontio/OpenFrontIO/blob/v0.34.11/src/core/execution/NukeExecution.ts)
- [SAMLauncherExecution.ts: interception planning and priority](https://github.com/openfrontio/OpenFrontIO/blob/v0.34.11/src/core/execution/SAMLauncherExecution.ts)
- [SAMMissileExecution.ts: target whitelist and interceptor behavior](https://github.com/openfrontio/OpenFrontIO/blob/v0.34.11/src/core/execution/SAMMissileExecution.ts)
- [PlayerImpl.ts: costs and ready Silo selection](https://github.com/openfrontio/OpenFrontIO/blob/v0.34.11/src/core/game/PlayerImpl.ts)
- [UnitImpl.ts: independent Silo and SAM slot queues](https://github.com/openfrontio/OpenFrontIO/blob/v0.34.11/src/core/game/UnitImpl.ts)
