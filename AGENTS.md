...
- **在 linked worktree 中运行 `gh pr merge --delete-branch` 可能已完成远端合并，却因本地 `main` 被另一个 worktree 占用而以退出码 1 结束**：看到 `fatal: 'main' is already used by worktree` 时先用 REST 核对 PR 的 `merged`、`merge_commit_sha` 与远端 main；确认已合并后不要重跑 merge，只通过 GitHub ref API 删除已核准的远端主题分支，本地 worktree/分支另行清理。自动化在 linked worktree 中优先用 REST 完成 squash merge 与远端分支删除，避免隐式 checkout。
