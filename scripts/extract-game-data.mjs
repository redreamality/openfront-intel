...
const UNIT_SNAPSHOT = [
  ...
  },
  {
    ...
    upgradable: false,
    i18n: {
      en: { name: 'MIRV', role: 'Long-range multi-warhead vehicle; splits into multiple MIRV Warheads on arrival.', costFormula: '25,000,000 + 15,000,000 × game-wide MIRVs launched', notes: ['Major v24 nerf: craters can be quickly conquered', 'No global launch lockout in v0.34.11; each Silo tube reloads separately'] },
      zh: { name: 'MIRV 多弹头', role: '远程多弹头载具,到达目标后分裂为多枚 MIRVWarhead。', costFormula: '25,000,000 + 15,000,000 × 本局全局 MIRV 发射数', notes: ['v24 大幅削弱：陨石坑可被快速征服', 'v0.34.11 无全局发射锁定；每根 Silo 发射管独立装填'] },
      fr: { name: 'MIRV', role: "Véhicule longue portée à têtes multiples ; se divise en plusieurs têtes MIRV à l'arrivée.", costFormula: '25 000 000 + 15 000 000 × MIRV lancés dans la partie', notes: ['Gros nerf v24 : les cratères peuvent être conquis rapidement', 'Aucun blocage global des tirs en v0.34.11 ; chaque tube de Silo recharge séparément'] },
      de: { name: 'MIRV', role: 'Langstrecken-Mehrkopfraketenträger; teilt sich bei Ankunft in mehrere MIRV-Sprengköpfe.', costFormula: '25.000.000 + 15.000.000 × weltweite MIRV-Starts', notes: ['Großer v24-Nerf: Krater können schnell erobert werden', 'Keine globale Startsperre in v0.34.11; jedes Silo-Rohr lädt separat nach'] },
      nl: { name: 'MIRV', role: 'Langeafstandsvoertuig met meerdere kernkoppen; splitst bij aankomst in meerdere MIRV-kernkoppen.', costFormula: '25.000.000 + 15.000.000 × wereldwijde MIRV-lanceringen', notes: ['Grote v24-nerf: kraters kunnen snel veroverd worden', 'Geen wereldwijde lanceerblokkade in v0.34.11; elke Silo-buis herlaadt apart'] },
    },
  },
  {
    id: 'MIRVWarhead',
...
const FORMULAS_SNAPSHOT = {
  ...
  troops: {
    ...
    items: [
      { i18n: { en: { name: 'Population cap base', expr: '2 × (tiles^0.6 × 1000 + 50,000) + Σ(cityLevel × 250,000)' }, zh: { name: '人口上限基础', expr: '2 × (tiles^0.6 × 1000 + 50,000) + Σ(cityLevel × 250,000)' }, fr: { name: 'Base limite de population', expr: '2 × (tiles^0.6 × 1000 + 50 000) + Σ(cityLevel × 250 000)' }, de: { name: 'Bevölkerungslimit-Basis', expr: '2 × (tiles^0.6 × 1000 + 50.000) + Σ(cityLevel × 250.000)' }, nl: { name: 'Bevolkingslimiet-basis', expr: '2 × (tiles^0.6 × 1000 + 50.000) + Σ(cityLevel × 250.000)' } } },
  ...
  nukes: {
    ...
    items: [
      ...
      { i18n: { en: { name: 'MIRV price ladder', expr: '25M + 15M × game-wide prior launches; no global launch lockout' }, zh: { name: 'MIRV 价格阶梯', expr: '25M + 15M × 本局全局此前发射数；无全局发射锁定' }, fr: { name: 'Échelle de prix MIRV', expr: '25M + 15M × tirs mondiaux précédents ; aucun blocage global' }, de: { name: 'MIRV-Preisleiter', expr: '25M + 15M × bisherige weltweite Starts; keine globale Startsperre' }, nl: { name: 'MIRV-prijsladder', expr: '25M + 15M × eerdere wereldwijde lanceringen; geen wereldwijde blokkade' } } },
