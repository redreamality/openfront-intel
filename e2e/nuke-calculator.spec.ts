...
test('nuke calculator computes exact costs, ready slots, range, and updates live', async ({ page }) => {
  ...
  await expect(page.getByLabel('MIRVs planned')).toHaveValue('3');
  await expect(page.getByLabel('MIRVs already launched this game')).toHaveValue('0');
  ...
  await page.getByLabel('MIRVs planned').fill('1');
  await page.getByLabel('MIRVs already launched this game').fill('2');
  ...
  await expect(page.getByTestId('nuke-next-mirv')).toHaveAttribute('data-value', '55000000');
  await expect(page.getByTestId('nuke-mirv-series')).toHaveAttribute('data-value', '55000000');
  await expect(page.getByTestId('nuke-total-cost')).toHaveAttribute('data-value', '55000000');
  await expect(page.getByTestId('nuke-balance')).toHaveAttribute('data-value', '145000000');
  ...
  await page.getByLabel('MIRVs planned').fill('2');
  await expect(page.getByTestId('nuke-mirv-series')).toHaveAttribute('data-value', '125000000');
  await page.getByLabel('MIRVs planned').fill('0');
  await expect(page.getByTestId('nuke-mirv-series')).toHaveAttribute('data-value', '0');
});
...
for (const locale of localeCases) {
  test(`MIRV and calculator guidance is localized and explicit [${locale.lang}]`, async ({ page }) => {
    ...
    await expect(page.locator('main')).toContainText('v0.34.11');
