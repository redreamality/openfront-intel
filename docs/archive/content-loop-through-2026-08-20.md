...
定时任务每次运行都必须先读项目级 `AGENTS.md`、内容战略、本账本和 [`whats-new-content-plan.md`](../whats-new-content-plan.md)。不得把临时信号直接升级为长期路线，也不得因为队列第一项未完成就机械地重复加工同一页面。
...
2. `What's New` 已跟踪 Issue / PR 的状态转换，以及自上次 `main` 锚点以来的新合并 PR。
3. 更新过的高价值开放 PR、已批准 Issue，以及上游机制、默认键位、地图、数值和占领规则变化。
4. `.cache/gsc/` 中最近的 7 天和 28 天 Search Console 意图报告。
5. Feedlog、站点 Issues 和社区中重复出现的玩家问题。
6. `content-strategy.md` 当前战役与本文件的信号池。
...
7. 更新 `What's New 来源账本` 的 Release / `main` 双锚点、跟踪 ID、披露判断与下一决策点；没有实质变化时不得刷新公共条目的核验日期。
...
- 战役：用户明确启动 `WN-01`，为五语站点建立经过筛选的 `What's New` 栏目和 GitHub Issues / PRs 每日来源流程；正式非 TEST `v0.33.6` 的 `FRESH-01` 已完成。
- 玩家结果：玩家能区分“已经上线、已合并待发布、开发中、已批准观察”，并从更新摘要进入正式版本历史或对应常青主答案。
- 当前计划项：Production 为 `WN-01`；Monitoring 为空；Incubation 保留 `MAP-01` 与等待 #4237 或真实重复问题的 `ATTACK-01`。
- 完成条件：五语 `/whats-new/`、共享事实 schema、首批高价值条目、首页 / 导航入口和状态 e2e 全部交付；changelog 继续只承接正式 Release，安全 / 私有 / 未批准来源不公开。
...
- 用户明确要求增加 `What's New` 栏目，并把可访问的上游 Issues / PRs 纳入内容账本，因此允许即时调整内容战略。
- 本轮只完成一手来源探索、状态契约、首批候选和后续每日任务规划；不在证据与信息架构尚未固定时直接上线页面。
- `What's New` 不复制 GitHub 活动流。公开状态限定为 `Released`、`Merged`、`In development`、`Watching`；只有非 TEST Release 和 tag 能写“已上线”。
- 上游 `v0.33.6` 与 `main` 已分叉，不能用 release compare 的 104 个 ahead 提交作为日增量；本轮改用账本的 `0b0c765 → 9c97e01` 八提交区间。
- 下一 Production 批次按 [`whats-new-content-plan.md`](../whats-new-content-plan.md) 实施五语静态 MVP；开发中条目不提前改写正式机制，外部私有依赖和安全问题只内部观察。

## 当前进行中

- Production：`WN-01` 的来源研究、披露合同和实施计划已完成，下一批次实现五语静态栏目、共享 schema、首批条目、入口和 e2e。`FRESH-01` 的五语 v33.6 回放响应已由 [PR #36](https://github.com/redreamality/openfront-intel/pull/36) 合并为 `62bb30f`。
...
| ID | 状态 | 下一决策点 |
|---|---|---|
| `WN-01` | Production；一手来源探索、状态契约、首批候选和每日流程已写入 [`whats-new-content-plan.md`](../whats-new-content-plan.md)，公共页面尚未实现 | 下一内容批次完成共享 schema、五语 `/whats-new/`、首页 / 导航入口和 e2e；首批公开项不得越过其 GitHub 生命周期 |
...
- 扫描时间：2026-08-20。
- 最新正式 Release：[`v0.33.6`](https://github.com/openfrontio/OpenFrontIO/releases/tag/v0.33.6)，tag commit `bebc953`。
- 上游 `main` 日增量：`0b0c765 → 9c97e01`，8 个合并提交。
- 分叉边界：`v0.33.6...main` 为 `diverged`，不能把 ahead 104 / behind 20 解释成线性版本差异。
- 详细证据、排除项和页面合同：[`whats-new-content-plan.md`](../whats-new-content-plan.md) 与 [`2026-08-20-whats-new-upstream-source-plan.md`](../research/2026-08-20-whats-new-upstream-source-plan.md)。

| 主题 | 状态 | 披露判断 | 下一决策点 |
|---|---|---|---|
| Team Doomsday 更高门槛 + 腐化领土 wasteland | `Merged` | P0 公开首发候选；合并 [#5030](https://github.com/openfrontio/OpenFrontIO/pull/5030) 与 [#5052](https://github.com/openfrontio/OpenFrontIO/pull/5052) 为一个玩家主题 | 正式 Release 收录后晋级并同步五语 Doomsday 主答案 |
| Water Nukes 运输船寻路 | `Merged` | P0；现有五语攻略已正确标成未发布修复，来源 [#4975](https://github.com/openfrontio/OpenFrontIO/pull/4975) | Release 收录后转 `Released` 并移除旧绕路警告 |
| 完整大厅 Detailed View | `Merged` | P1 公开首发候选；来源 [#5022](https://github.com/openfrontio/OpenFrontIO/pull/5022) | Release 后核对最终入口、筛选项和公开队列行为 |
| Clan Treasury 捐赠 | `Merged` | P1 公开首发候选；来源 [#5041](https://github.com/openfrontio/OpenFrontIO/pull/5041)，必须保留永久且不可退款警告 | Release 后核对普通 / premium currency UI 与权限 |
| Spectator mode | `In development` | P0 开发中候选；[#5031](https://github.com/openfrontio/OpenFrontIO/pull/5031) 仍开放且 review required | 合并后进入公开 `Merged`；关闭或大改则撤下 |
| SAM 动态升级射程 | `In development` | P0 内部重点观察；[#5040](https://github.com/openfrontio/OpenFrontIO/pull/5040) 当前 blocked | 解除 blocked 并合并后准备核武机制同步 |
| Loadout、地图预加载、联盟操作盘、色觉可区分颜色 | `In development` | P1 观察；分别为 [#5050](https://github.com/openfrontio/OpenFrontIO/pull/5050)、[#5046](https://github.com/openfrontio/OpenFrontIO/pull/5046)、[#4974](https://github.com/openfrontio/OpenFrontIO/pull/4974)、[#4932](https://github.com/openfrontio/OpenFrontIO/pull/4932) | 只在评审稳定或合并后占用公开位置 |
| Ranked 2v2 固定队友 | `In development` | 外部依赖当前不可验证，只内部观察；来源 [#4788](https://github.com/openfrontio/OpenFrontIO/pull/4788) | 公开依赖和服务端状态可验证后再判断 |
| Exploit、认证、滥用、私有仓库、not-approved / DevOps / 重构 | `Suppressed` | 不公开；只记录状态和修复后的玩家结果 | 正式安全修复发布后才重新评估 |
...
- 2026-08-20｜`WN-01` 来源扩展｜用户明确要求建立 `What's New` 并把上游 Issues / PRs 纳入每日内容来源。本轮已区分 `Released`、`Merged`、`In development`、`Watching`，并确认 Release 与 `main` 分叉时不能把 merged 当 released；计划见 [`whats-new-content-plan.md`](../whats-new-content-plan.md)。
- 2026-08-20｜`WN-01` 首批候选｜P0 为 Team Doomsday 门槛 / wasteland 与 Water Nukes 寻路；P1 公开候选为完整大厅浏览和 Clan Treasury；开发中重点观察 Spectator 与 SAM 动态射程。全部未发布项继续等待正式 Release；安全 / 滥用、私有依赖、未批准 Issue 和开发噪声不公开。
...
## 本次合规自检（2026-08-20，`WN-01` 来源与规划）

- 计划 ID：`WN-01`。
- 是否推进当前战役：是。用户明确要求的新栏目已经形成状态契约、双游标、首批候选、公共 MVP 范围、实施批次和每日维护流程；代码实现进入下一 Production 批次。
- 为何未制造薄页：本轮没有直接把 27 个开放 PR 或 132 个开放 Issue 转成页面；同一玩家结果先合并为主题，未发布状态不改写正式攻略。
- 唯一主落地页：未来 `/whats-new/` 只做状态索引；正式版本事实仍由 changelog 承接，长解释仍进入 Doomsday、Water Nukes、economy / trade 等既有唯一主答案。
- 语言状态：公共页面尚未实现；计划要求 canonical 条目共享事实，并同时提供 en、zh、fr、de、nl 文案。
- 旧页同步范围：0。Doomsday、Water Nukes、Port、SAM 等变化仍未由正式 Release 确认，本轮不提前改写当前规则。
- 视觉证据类型：研究使用官方 Release、Issue、PR、commit、milestone、源码和 PR 自带截图；没有伪造游戏画面。
- 验证结果：研究来源包包含 58 个上游官方链接；Markdown 无 BOM、尾随空格和多余 EOF 空行；`git diff --check` 通过。docs-only，未运行 build、link check 或 e2e。
- 阻塞：GitHub milestone 辅助端点出现一次 `EOF`，部分并行 ref / 跨仓库请求出现 TLS 超时或不可见；已保留成功端点、旧游标和未验证状态，没有把失败输出写成 0 或已交付，并已追加对应避坑规则。
...
- [x] 2026-08-20：完成 `WN-01` 一手来源探索和栏目规划；建立 Release / `main` 双游标、四态披露合同、首批公开与内部候选、安全排除、五语 MVP、实施批次和每日状态迁移。公共页面代码留给下一 Production 批次。
...
| 2026-08-20 | `WN-01` 来源与规划 | 把上游 Releases、Issues 和 PRs 接成正式来源，完成状态契约、双游标、首批候选、公共五语 MVP 和每日任务规划；本轮不提前上线页面 | 正式 `v0.33.6` / `bebc953` 与 `main` `9c97e01` 分叉；真实日增量为 `0b0c765 → 9c97e01` 八提交；筛出 Doomsday、Water Nukes、大厅、Clan、Spectator、SAM 等高价值主题 | docs-only：58 个官方来源链接；BOM / 尾随空格 / EOF 与 `git diff --check` 通过；未跑产品 build / e2e | 下一 Production 批次实施共享 schema、五语 `/whats-new/`、首页 / 导航入口和状态 e2e |
