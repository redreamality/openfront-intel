...
| Production | `LF-COMMUNITY-ROLLING` | 本轮 `/guides/annexation-enclosure/` 已达到五语内容与来源门槛；合并即完成本次滚动主题并继续活动 | 下一轮重新从社区信号批准唯一意图；不得把兼并包围与威胁评估、陆战或四岛路线协同换标题重复生产。 |
...
- 正式 Release：[`v0.33.13`](https://github.com/openfrontio/OpenFrontIO/releases/tag/v0.33.13)；公开 FFA 默认启用 Overtime，未新增兼并规则；车站断开修复与铁路收益边界继续有效。
- 上游 `main` 游标：`a59ded2977490c3d482d2f65bb248d90f26bfd3f`；本轮已用 `git ls-remote` 核验。SAM 动态升级射程、地图预加载与外观 loadout 仍保持 Merged，不能越过正式 Release 边界。
...
| 2026-09-03 | `LF-COMMUNITY-ROLLING` Four Islands 团队协同 | 新建五语 `/guides/four-islands-team-coordination/`，回答四岛分区、路线账本、桥梁/锚点/压力/储备职责、运输窗口、双队与四队核防守场景；接通五语 Guides、island-defense 与 team-roles | 5 Reddit、3 YouTube、10 个官方来源；guide audit 五语与研究门槛通过；合并即完成本轮主题并继续滚动 Production |
| 2026-09-04 | `LF-COMMUNITY-ROLLING` 兼并与包围节奏 | 新建五语 `/guides/annexation-enclosure/`，回答小额试探、内陆集群闭合、活动攻击归属、地形成本、两类数字场景、失败反制与地图/模式调整；接通五语 Guides、threat-assessment 与 land-combat | 3 Reddit、3 YouTube、4 个官方来源；guide audit 五语与研究门槛通过；合并即完成本轮主题并继续滚动 Production |
