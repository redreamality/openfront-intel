...
- **2026-08-23 再次复发：**本环境到 github.com:443 的 git 传输不稳定，本轮 `git fetch` 和首次 `git push` 都报 `Recv failure: Connection was reset`，唯一一次 push 重试又报 `Failed to connect to github.com port 443`。每次失败后都先用 GitHub ref API 核对远端；两次 push 均未更新时停止 Git HTTPS，并改用 Git Data API做同一项 fast-forward 交付。需要核对远端基线或上游 diff 时优先走 GitHub API，不要依赖 `git pull`；git 命令一律加 `-c http.lowSpeedLimit=1000 -c http.lowSpeedTime=25` 快速失败，避免无限挂起。
