...
- **2026-09-25 再次复发：同一轮不要并发多个 `gh api` 大列表或 compare 请求**：本环境并发访问 GitHub API 会让原本可用的端点一起报 `TLS handshake timeout`，单个宽 compare 也可能 30 秒无输出。改为逐个顺序调用并缩小 compare 范围，每个端点只重试一次，并在 PowerShell 管道后立即保存、检查 `$LASTEXITCODE`；任一请求失败时保留旧游标，不能把缺失输出解释为 0 条 Issue / PR / commit。
