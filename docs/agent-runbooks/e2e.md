...
- **2026-09-14 复发：多语攻略 e2e 的意图断言不能假定整句与源码逐字一致**：法语 hotkeys 页面已包含训练、战斗流程和错误恢复语义，但测试使用的整句片段因当前译文措辞不同而失败。新增断言应选渲染后稳定的短语或有限同义正则，保留页面职责的关键语义即可。
- **2026-09-15 复发：重组攻略目录时要保留多语元数据标签契约**：法语目录的日期前缀由既有 e2e 锁定为“Mis à jour le”；共享目录组件若只写“Mis à jour”会让内容正确但回归失败。迁移目录卡片时同步核对各语种更新/发布标签与现有断言。
- **正文回链与 RelatedContent 的数量不是同一契约**：同一目标可能在正文出现多次，且页尾推荐再加一条。发现性 e2e 应限定 `main .prose` 并断言至少一条可见目标链接；只有产品明确要求唯一入口时才使用 `toHaveCount(1)`。2026-09-20 Impossible 五语回链测试曾因把两处有效正文链接视为失败而误报。
