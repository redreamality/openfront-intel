...
- **2026-08-24 再次复发：读取源码、内容页、配置或审计器前不得猜路径或文件名**：本轮又误用不存在的 `src/i18n/utils.ts`，并把不存在的候选配置路径与有效根一起传给 `rg`。先从最近的真实目录运行 `rg --files`，再把返回路径传给 `Get-Content` 或 `rg`；不要把猜测路径与有效路径放在同一命令中。若搜索表达式本身以 `-` 开头，必须在选项后加 `--` 终止参数解析，例如 `rg -n -- '--color' <已确认目录>`。
